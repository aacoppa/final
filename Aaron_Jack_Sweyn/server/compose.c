...
char * compose_init_tables() {
    ...
    strcpy(execStr, "CREATE TABLE users ( username TEXT, password TEXT); CREATE TABLE game_data ( u1 TEXT, u2 TEXT, turn INTEGER, u1wins INTEGER, u2wins INTEGER, key BLOB, dist INTEGER);");
    return execStr;
}
